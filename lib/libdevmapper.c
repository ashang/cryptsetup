...
static int _dm_check_versions_all(const struct crypt_dm_active_device *dmd)
{
	int i, r = 1;

	if (!dmd)
		return _dm_check_versions(DM_UNKNOWN);

	for (i = 0; i < dmd->segment_count; i++)
		if (!_dm_check_versions(dmd->segment[i].type))
			r = 0;
...
int dm_flags_all(const struct crypt_dm_active_device *dmd, uint32_t *flags)
{
	...
	for (i = 0; i < dmd->segment_count; i++) {
		if ((r = dm_flags(dmd->segment[i].type, &dmt_flags)))
			return r;
		*flags |= dmt_flags;
	}
 ...
 */
static int dm_init_context(struct crypt_device *cd, const struct crypt_dm_active_device *dmd)
{
	_context = cd;
	if (!_dm_check_versions_all(dmd)) {
		if (getuid() || geteuid())
...
static char *get_dm_crypt_params(const struct dm_target *tgt, uint32_t flags)
{
	...
	if (flags & CRYPT_ACTIVATE_KEYRING_KEY) {
		keystr_len = strlen(tgt->u.crypt.vk->key_description) + get_key_size_strlen(tgt->u.crypt.vk->keylength) + 9;
...
static char *get_dm_integrity_params(const struct dm_target *tgt, uint32_t flags)
{
	...
	max_size = strlen(device_block_path(tgt->data_device)) +
			...
			(tgt->u.integrity.journal_crypt ? strlen(tgt->u.integrity.journal_crypt) : 0) +
			128;
...
static char *get_dm_linear_params(const struct dm_target *tgt, uint32_t flags)
{
	char *params;
	int r;
	int max_size = strlen(device_block_path(tgt->data_device)) + 64;
...
int dm_remove_device(struct crypt_device *cd, const char *name, uint32_t flags)
{
	...
	(void)dm_flags(DM_UNKNOWN, &dmt_flags);
...
static void _destroy_dm_target_params(struct dm_target *tgt)
{
	if (tgt->type == DM_LINEAR)
		free(tgt->params);
	else
		crypt_safe_free(tgt->params);
...
static void _destroy_dm_targets_params(struct crypt_dm_active_device *dmd)
{
	...
	for (i = 0; i < dmd->segment_count; i++)
		_destroy_dm_target_params(&dmd->segment[i]);
}
...
static int _dm_reload_device(const char *name,
			     const char *type,
			     struct crypt_dm_active_device *dmd)
{
	int r = -EINVAL;
...
/* TODO: anything to do with multi-segment devices? */
#ifdef DM_READ_AHEAD_MINIMUM_FLAG
	if (dmd->segment_count == 1) {
		if (device_read_ahead(dmd->segment[0].data_device, &read_ahead) &&
			     ...
			     const char *uuid, struct crypt_dm_active_device *dmd)
{
	...
	/* All devices must have DM_UUID, only resize on old device is exception */
	if (!dm_prepare_uuid(name, type, dmd->uuid, dev_uuid, sizeof(dev_uuid)))
		goto out;
	...
	/* only if dm_task_get_info() failed */
	if (r < 0)
		_dm_remove(name, 1, 0);
...
int at_least_one_dm_target_is(struct crypt_dm_active_device *dmd, dm_target_type type)
{
	int i;

	for (i = 0; i < dmd->segment_count; i++)
		if (dmd->segment[i].type == type)
			return 1;
...
static void _dm_target_free_query_path(struct dm_target *tgt)
{
	switch(tgt->type) {
	case DM_CRYPT:
	...
	case DM_VERITY:
		...
		device_free(tgt->u.verity.hash_device);
	...
	device_free(tgt->data_device);
}

static void _dm_target_free(struct dm_target *tgt)
{
	switch(tgt->type) {
	case DM_CRYPT:
	...
	case DM_VERITY:
		/* fall through */
	case DM_INTEGRITY:
		crypt_safe_free(tgt->params); /* may hold journal keys */
		break;
	case DM_LINEAR:
		free(tgt->params);
		break;
...
static void _dm_target_erase(struct dm_target *tgt)
{
	if (!tgt)
		return;

	if (tgt->direction == TARGET_QUERY)
		_dm_target_free_query_path(tgt);
...
void dm_targets_free(struct crypt_dm_active_device *dmd)
{
	...
	for (i = 0; i < dmd->segment_count; i++)
		_dm_target_erase(&dmd->segment[i]);
		     ...
		     struct crypt_dm_active_device *dmd)
{
	...
	r = _dm_reload_device(name, type, dmd);

	if (r == -EINVAL &&
	    at_least_one_dm_target_is(dmd, DM_CRYPT) &&
	    (dmd->flags & (CRYPT_ACTIVATE_SAME_CPU_CRYPT|CRYPT_ACTIVATE_SUBMIT_FROM_CRYPT_CPUS)) &&
	    !dm_flags(DM_CRYPT, &dmt_flags) && !(dmt_flags & (DM_SAME_CPU_CRYPT_SUPPORTED|DM_SUBMIT_FROM_CRYPT_CPUS_SUPPORTED)))
		     ...
		     struct crypt_dm_active_device *dmd)
{
	...
	r = _dm_create_device(name, type, dmd->uuid, dmd);

	if (r < 0 && dm_flags_all(dmd, &dmt_flags))
		goto out;

	if (r && at_least_one_dm_target_is(dmd, DM_CRYPT) && check_retry(&dmd->flags, dmt_flags))
		r = _dm_create_device(name, type, dmd->uuid, dmd);
	...
	/* TODO fix this, dm-verity is always only one */
	if (r == -EINVAL && at_least_one_dm_target_is(dmd, DM_VERITY) && dmd->segment[0].u.verity.fec_device &&
	    !(dmt_flags & DM_VERITY_FEC_SUPPORTED))
		log_err(cd, _("Requested dm-verity FEC options are not supported.\n"));

	/* TODO fix this. integrity detection has to be done differently */
	/* has_integrity, has_sector_size etc */
	if (r == -EINVAL && at_least_one_dm_target_is(dmd, DM_CRYPT)) {
		if (dmd->segment[0].u.crypt.integrity && !(dmt_flags & DM_INTEGRITY_SUPPORTED))
			log_err(cd, _("Requested data integrity options are not supported.\n"));
		if (dmd->segment[0].u.crypt.sector_size != SECTOR_SIZE && !(dmt_flags & DM_SECTOR_SIZE_SUPPORTED))
...
/* TODO this is not multi-seg ready */
...
static int _dm_target_query_crypt(uint32_t get_flags,
...
static int _dm_target_query_verity(uint32_t get_flags,
			           char *params,
			           struct dm_target *tgt,
				   uint32_t *act_flags)
{
...
static int _dm_target_query_integrity(uint32_t get_flags,
			     char *params,
			     struct dm_target *tgt,
			     uint32_t *act_flags)
{
...
static int _dm_target_query_linear(struct dm_target *tgt, uint32_t get_flags,
				   char *params)
{
	uint64_t val64;
	char *rdevice, *arg;
 ...
 */
static int dm_target_query(struct dm_target *tgt, const uint64_t *start,
		    ...
		    char *params, uint32_t get_flags, uint32_t *act_flags)
{
	...
	if (!strcmp(target_type, DM_CRYPT_TARGET))
		r = _dm_target_query_crypt(get_flags, params, tgt, act_flags);
	else if (!strcmp(target_type, DM_VERITY_TARGET))
		r = _dm_target_query_verity(get_flags, params, tgt, act_flags);
	else if (!strcmp(target_type, DM_INTEGRITY_TARGET))
		r = _dm_target_query_integrity(get_flags, params, tgt, act_flags); /* TODO */
	else if (!strcmp(target_type, DM_LINEAR_TARGET))
		r = _dm_target_query_linear(tgt, get_flags, params);
		    ...
		    uint32_t get_flags, struct crypt_dm_active_device *dmd)
{
	...
	r = 0;
	do {
		t = &dmd->segment[dmd->segment_count++];

		next = dm_get_next_target(dmt, next, &start, &length,
	                                  &target_type, &params);

		/*
		 * all master keys in any dm-crypt segment params have to be purged
		 * therefore iterate over all items even on error
		 */
		_r = dm_target_query(t, &start, &length, target_type, params, get_flags, &dmd->flags);
		if (!_r && t->type == DM_VERITY) {
			_r = _dm_status_verity_ok(name);
			if (_r == 0)
				dmd->flags |= CRYPT_ACTIVATE_CORRUPTED;
		}

		if (_r < 0) {
			log_err(cd, _("Failed to query %u. segment.\n"), dmd->segment_count);
			r = _r;
		}
